...
public static class ServiceCollectionExtensions
{
        ...
        Assembly serviceAssembly)
    {
        ...
        foreach (Type serviceClass in serviceClasses)
        {
            ...
            {
                throw new ArgumentException($"Interface {serviceClass.Name} not found for service");
            }
        ...
        Assembly repositoryAssembly)
    {
        Type[] repositoryClasses = repositoryAssembly
            .GetTypes()
            .Where(t => t.Name.EndsWith(ProjectRepositorySuffix) &&
                        !t.IsInterface &&
                        !t.IsAbstract)
            .ToArray();
        foreach (Type repositoryClass in repositoryClasses)
        {
            ...
            {
                // Register with interface if it exists
                serviceCollection.AddScoped(repositoryInterface, repositoryClass);
            }
            else
            {

                serviceCollection.AddScoped(repositoryClass);
            }
        }
